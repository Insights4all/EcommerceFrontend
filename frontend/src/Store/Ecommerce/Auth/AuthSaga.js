...
function* UserLogin(action) {
  ...
  if (response) {
    localStorage.setItem("token", response.data.accessToken);
    yield put({ type: AuthActionTypes.USER_LOGIN_SUCCESS, response });
  } else yield put({ type: AuthActionTypes.USER_LOGIN_FAILED, error });
}
...
function* ShopLogin(action) {
  const { response, error } = yield call(AuthService.ShopLogin, action.payload);
  console.log("response", response);

  if (response) {
    localStorage.setItem("token", response.data.accessToken);
    yield put({ type: AuthActionTypes.SHOP_LOGIN_SUCCESS, response });
  } else yield put({ type: AuthActionTypes.SHOP_LOGIN_FAILED, error });
}

function* ShopRegister(action) {
  try {
    const res = yield call(AuthService.ShopRegister, action.payload);
    console.log("dsafa", res);

    if ([1, 200, 201].includes(res.status)) {
      localStorage.setItem("token", res.data.accessToken);
      // const payload = yield call(AuthParser.GetAllProducts, res);
      yield put({ type: AuthActionTypes.SHOP_REGISTER_SUCCESS, res });
    } else {
      console.log("success");

      yield put({ type: AuthActionTypes.SHOP_REGISTER_FAILED });
    }
  } catch (error) {
    console.log(`Action ${action.type} failed with ${error}`);
  }
...
export function* AuthSaga() {
  ...
  yield takeLatest(AuthActionTypes.SHOP_LOGIN_REQUEST, ShopLogin);
  yield takeLatest(AuthActionTypes.SHOP_REGISTER_REQUEST, ShopRegister);
}
